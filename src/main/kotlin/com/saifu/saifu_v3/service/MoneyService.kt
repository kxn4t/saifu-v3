...
class MoneyService(
    ...
    fun fetchAllMoneyList(): List<Money> = zaimClient.getMoneyList()
    ...
    fun calculate(condition: CalculateForm): CalculateResult {
        ...
        // 期間内のMoneyListを取得
        val calculateSource = fetchSixMonthsMoneyList().filter {
        ...
        val wrongList = calculateSource.filter {
            (it.comment != condition.keyword?.get(0)) && (it.comment != condition.keyword?.get(1))
        }
        ...
        // 差分計算して結果を格納
        val average = sumResults.map { it.value }.sum() / 2
    ...
    /** ここ半年間のものを取得する */
    private fun fetchSixMonthsMoneyList(): List<Money> {
        val sixMonthsAgo = LocalDate.now().minusMonths(6).format(dateTimeFormatter)
